...
if [ -d "$HOME/ansible-wireguard" ]; then
  pushd $HOME/ansible-wireguard
  git pull
  popd
else
  git clone https://github.com/carriergrade/ansible-wireguard $HOME/ansible-wireguard
fi
...
set -e
cd $HOME/ansible-wireguard
[ -d $HOME/ansible-wireguard/.venv ] || $PYTHON -m venv .venv
export VIRTUAL_ENV="$HOME/ansible-wireguard/.venv"
export PATH="$HOME/ansible-wireguard/.venv/bin:$PATH"
.venv/bin/python3 -m pip install --upgrade pip
.venv/bin/python3 -m pip install -r requirements.txt


# Install the Galaxy requirements
cd $HOME/ansible-wireguard && ansible-galaxy install --force -r requirements.yml
...
touch $HOME/ansible-wireguard/custom.yml

custom_filled=$(awk -v RS="" '/username/&&/dns_nameservers/&&/root_host/{print FILENAME}' $HOME/ansible-wireguard/custom.yml)
...
if [[ "$custom_filled" =~ "custom.yml" ]]; then
  ...
  echo
  cd $HOME/ansible-wireguard && ansible-playbook --ask-vault-pass run.yml
  exit 0
fi

clear
echo "Welcome to ansible-wireguard!"
echo
...
echo "username: \"${username}\"" >> $HOME/ansible-wireguard/custom.yml
...
if [[ "$adguard_enable" =~ ^[yY]$ ]]; then
  echo "enable_adguard_unbound_doh: true" >> $HOME/ansible-wireguard/custom.yml
fi
...
echo "root_host: \"${root_host}\"" >> $HOME/ansible-wireguard/custom.yml
...
echo "dns_nameservers: \"${dns_nameservers}\"" >> $HOME/ansible-wireguard/custom.yml
...
if [[ ! $AWS_EC2 =~ true ]]; then
  ...
  echo "enable_ssh_keygen: true" >> $HOME/ansible-wireguard/custom.yml

  if [[ "$new_ssh_key_pair" =~ ^[yY]$ ]]; then
    echo
    read -p "Please enter your SSH public key: " ssh_key_pair

    echo "ssh_public_key: \"${ssh_key_pair}\"" >> $HOME/ansible-wireguard/custom.yml
  fi
fi
...
if [[ "$email_setup" =~ ^[yY]$ ]]; then
  ...
  if [ ! -z ${email} ]; then
    echo "email: \"${email}\"" >> $HOME/ansible-wireguard/custom.yml
  fi

  read -p "'To' e-mail [${email_login}]: " email_recipient
  if [ ! -z ${email_recipient} ]; then
    echo "email_recipient: \"${email_recipient}\"" >> $HOME/ansible-wireguard/custom.yml
  fi



  echo "email_smtp_host: \"${email_smtp_host}\"" >> $HOME/ansible-wireguard/custom.yml
  echo "email_smtp_port: \"${email_smtp_port}\"" >> $HOME/ansible-wireguard/custom.yml
  echo "email_login: \"${email_login}\"" >> $HOME/ansible-wireguard/custom.yml
fi


# Set secure permissions for the Vault file
touch $HOME/ansible-wireguard/secret.yml
chmod 600 $HOME/ansible-wireguard/secret.yml

if [ -z ${email_password+x} ]; then
  echo
else 
  echo "email_password: \"${email_password}\"" >> $HOME/ansible-wireguard/secret.yml
fi

if [[ $user_password =~ '"' ]]; then
  echo "user_password: '${user_password}'" >> $HOME/ansible-wireguard/secret.yml
else
  echo "user_password: \"${user_password}\"" >> $HOME/ansible-wireguard/secret.yml
fi
...
echo "jwt_secret: ${jwt_secret}" >> $HOME/ansible-wireguard/secret.yml
echo "session_secret: ${session_secret}" >> $HOME/ansible-wireguard/secret.yml
echo "storage_encryption_key: ${storage_encryption_key}" >> $HOME/ansible-wireguard/secret.yml

echo
echo "Encrypting the variables"
ansible-vault encrypt $HOME/ansible-wireguard/secret.yml
...
if [[ "$launch_playbook" =~ ^[yY]$ ]]; then
  if [[ $EUID -ne 0 ]]; then
    ...
    cd $HOME/ansible-wireguard && ansible-playbook --ask-vault-pass -K run.yml
  else
    cd $HOME/ansible-wireguard && ansible-playbook --ask-vault-pass run.yml
  fi
else
  echo "You can run the playbook by executing the bootstrap script again:"
  echo "cd ~/ansible-wireguard && bash bootstrap.sh"
  exit
fi
